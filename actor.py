...
class ActorNet(object):
    def __init__(self,sess,env,name,params,train=True):
        ...
        with tf.name_scope(self.name):
            ...
            self.gradients_placeholder = tf.placeholder(tf.float32, [None, params["actionsize"]],name="gradient_plh")
        ...
        self.actor_gradients = tf.gradients(self.scaled_out, self.params_list, -self.gradients_placeholder)
    ...
    def buildLowDimNet(self):
        ...
        with tf.name_scope(self.name):
            ...
            with tf.name_scope('output'):
                self.W_fc3 = self._weight_variable([300, self.params["actionsize"]],"W_out",vals=(-1./np.sqrt(300),1./np.sqrt(300)))
                self.params_list.append(self.W_fc3)
                
                self.b_fc3 = self._bias_variable([self.params["actionsize"]],"b_out",vals=(-1./np.sqrt(300),1./np.sqrt(300)))
                self.params_list.append(self.b_fc3)
                
            self.out=tf.nn.tanh(tf.matmul(h_fc2, self.W_fc3) + self.b_fc3)
            self.scaled_out=tf.multiply(float(self.params["action_bound"]),self.out)
    ...
    def policy(self,state_feed):
        action=self.sess.run(self.scaled_out,feed_dict={self.input_placeholder:state_feed})
        return action
